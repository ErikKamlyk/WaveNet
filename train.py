...
def run_training(epochs):
    for epoch in range(epochs):
        ...
        i = 0
        print('Epoch', epoch)
        for audio_b, mels_b, text_b in dataloader_train:
            if i%20 == 0:
                for param_group in optimizer.param_groups:
                    print(param_group['lr'])
            audio_b, mels_b, text_b = audio_b.to(device), mels_b.to(device), text_b.to(device)
            ...
            if i % 20 == 0:
                print(epoch, i, np.mean(train_losses[-20:]))
            if i  == (len(dataloader_train) - 1):
                if epoch % 9 == 8:
                    inference_fast(mels_b[:2], audio_b[:2], epoch=epoch)
                val(epoch)
                generator.train()
            i += 1
        ...
        wandb.log({"Train loss": np.mean(train_losses)}, step=epoch)
